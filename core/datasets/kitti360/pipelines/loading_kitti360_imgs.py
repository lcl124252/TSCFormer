...
class LoadMultiViewImageFromFiles_KITTI360(object):
    ...
    def get_inputs(self, results, flip=None, scale=None):
        ...
        if self.load_stereo_depth:
            stereo_depth_paths = results['stereo_depth_path']
            for i in range(len(stereo_depth_paths)):
                stereo_depth_path = stereo_depth_paths[i]
                ...
                stereo_depth = self.ToTensor(stereo_depth)
                stereo_depths.append(stereo_depth)
            stereo_depth = torch.stack(stereo_depths, dim=1)
            results['stereo_depth'] = stereo_depth.squeeze(0)
