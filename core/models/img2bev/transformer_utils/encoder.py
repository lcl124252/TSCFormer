        ...
        for layer in self.operation_order:
            ...
            if layer == 'self_attn':
                ...
                query = self.attentions[attn_index](
                    ...
                    prev_bev,
                    prev_bev,
                    identity if self.pre_norm else None,    # identity is None
                    query_pos=bev_pos,  # query_pos is None
                    key_pos=bev_pos,    # key_pos is None
                    attn_mask=attn_masks[attn_index],   # attn_mask is None
                    key_padding_mask=query_key_padding_mask,    # key_padding_mask is None
                    reference_points=ref_2d,    # ref_2d is None
                    spatial_shapes=torch.tensor(
                        [[bev_h, bev_w]], device=query.device),
                    level_start_index=torch.tensor([0], device=query.device),   # level_start_index is None
                    **kwargs)
                attn_index += 1
                identity = query
            ...
            # spaital cross attention
                ...
                query = self.attentions[attn_index](
                    ...
                    query_pos=query_pos,    # query_pos is None
                    key_pos=key_pos,    # key_pos is None
                    reference_points=ref_3d,    # ref_3d is None
                    reference_points_cam=reference_points_cam,  # reference_points_cam is None
                    mask=mask,  # mask is None
                    attn_mask=attn_masks[attn_index],   # attn_mask is None
                    key_padding_mask=key_padding_mask,  # key_padding_mask is None
                    spatial_shapes=spatial_shapes,  # spatial_shapes is None
                    level_start_index=level_start_index,    # level_start_index is None
                    **kwargs)
                attn_index += 1
                identity = query
