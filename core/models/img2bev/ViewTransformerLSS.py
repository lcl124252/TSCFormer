...
from .DeformAlign import FeatureAlign
...
class ViewTransformerLSS(BaseModule):
    def __init__(
        ...
        self.deform_align = FeatureAlign(fea_dim=128)
        self.DAhead = DANetHead(128, 128)
    ...
    def voxel_pooling(self, geom_feats, x):
        ...
        x = x[kept].to(device)
        geom_feats = geom_feats[kept].to(device).requires_grad_(False)
        # [b, c, z, x, y] == [b, c, x, y, z]
    ...
    def forward(self, feat, depth_prob, cam_params):
        ...
        # if N > 1:
        #     feat_aligned1 = self.deform_align(feat[:, 0:1, ...], feat[:, 1:2, ...])
        #     feat_aligned2 = self.deform_align(feat[:, 0:1, ...], feat[:, 2:3, ...])
        # feat0 = self.DAhead(feat[:, 0, ...]).unsqueeze(1)
        # feat1 = self.DAhead(feat_aligned1.squeeze(1)).unsqueeze(1)
        # feat2 = self.DAhead(feat_aligned2.squeeze(1)).unsqueeze(1)
        # feat = torch.cat((feat0, feat1, feat2), dim=1)
        ...
        for i in range(1, N):
            feat_aligned = self.deform_align(feat[:, 0:1, ...], feat[:, i:i + 1, ...])
            feat_processed = self.DAhead(feat_aligned.squeeze(1)).unsqueeze(1)
            feat_list.append(feat_processed)
        ...
        volume = volume.permute(0, 1, 3, 4, 5, 2)   # [B, N, 112, 48, 160, 128]
