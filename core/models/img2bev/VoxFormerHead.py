...
class VoxFormerHead(nn.Module):
    ...
    def forward(self, mlvl_feats, proposal, cam_params, lss_volume=None, img_metas=None,  **kwargs):
        ...
        volume_queries = self.volume_embed.weight.to(dtype).to(device)
        ...
        zeros = torch.zeros((bs, 512, 512), device=volume_queries.device, dtype=dtype)
        bev_pos_cross_attn = self.positional_encoding(zeros).to(dtype) # [1, dim, 128*4, 128*4]
        bev_pos_self_attn = self.positional_encoding(torch.zeros((bs, 512, 512), device=volume_queries.device, dtype=dtype)).to(dtype) # [1, dim, 128*4, 128*4]
