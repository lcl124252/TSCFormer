...
from .modules.Stereo_Depth_Net_modules import SimpleUnet, convbn_2d, temporalDepthAggregation
import pdb
...
class GeometryDepth_Net(BaseModule):
    def __init__(
        ...
        self.depth_aggregation = temporalDepthAggregation(embed_dims=32, out_channels=1)
    ...
    def get_mlp_input(self, rot, tran, intrin, post_rot, post_tran, bda=None):
        ...
        bda = bda.view(B, 1, *bda.shape[-2:]).expand(B, N, *bda.shape[-2:])
    ...
    def forward(self, input, img_metas):
        ...
        DB, DN, DH, DW = stereo_depth.shape
        ...
        stereo_volume = stereo_volume.view(DB * DN, H, W, -1).permute(0, 3, 1, 2)   # [N, 112, 48, 160]
        stereo_volume = self.stereo_volume_encoder(stereo_volume)   # [N, 112, 48, 160]
        stereo_volume = self.get_depth_dist(stereo_volume)  # [N, 112, 48, 160]

        depth_volume = self.depth_aggregation(stereo_volume, mono_volume)
        depth_volume = self.get_depth_dist(depth_volume)    # [N, 112, 48, 160]
