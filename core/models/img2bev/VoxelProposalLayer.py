...
class VoxelProposalLayer(BaseModule):
    ...
    def depth2lidar(self, image_grid, depth, cam_params):
        ...
        params = [param[:, 0:1, ...] for param in cam_params[:5]]
        bda = cam_params[5]

        rots, trans, intrins, post_rots, post_trans = params
        points = torch.cat([image_grid.repeat(b, 1, 1, 1), depth[:, 0:1, ...]], dim=1) # [b, 3, h, w] 取当前帧的深度图
    ...
    def forward(self, cam_params, img_metas):
        depth = img_metas['stereo_depth']   #[1, 5, 384, 1280]
        points = self.depth2lidar(self.image_grid, depth, cam_params)   # self.image_grid = [1, 2, 384, 1280]
