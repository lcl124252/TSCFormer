...
class TSCFormer(BaseModule):
    def __init__(
        ...
        for params in self.img_backbone.parameters():
    ...
    def extract_img_feat(self, img_inputs, img_metas):
        img = img_inputs[0]     # img_inputs来自loading_kitti_imgs.py [B,N,3,384,1280] B一般为1 N为帧数
        img_enc_feats = self.image_encoder(img) # [1,N,640,48,160]

        rots, trans, intrins, post_rots, post_trans, bda = img_inputs[1:7] # [1,N,3,3] [1,N,3] [1,N,4,4] [1,N,3,3] [1,N,3] [1,4,4]
        mlp_input = self.depth_net.get_mlp_input(rots, trans, intrins, post_rots, post_trans, bda)  # [1,N,33]
        geo_inputs = [rots, trans, intrins, post_rots, post_trans, bda, mlp_input]
        context, depth = self.depth_net([img_enc_feats] + geo_inputs, img_metas)    # [1,N,128,48,160] [N,112,48,160]
        ...
        if self.img_view_transformer is not None:
            lss_volume = self.img_view_transformer(context, depth, view_trans_inputs)   # [1,128,128,128,16]
        else:
            lss_volume = None
        query_proposal = self.proposal_layer(view_trans_inputs, img_metas)  # [1,1,128,128,16]
        ...
        if depth is not None:
            mlvl_dpt_dists = [depth.unsqueeze(1)]   # [N, 1, 112, 48, 160]
        else:
            mlvl_dpt_dists = None
