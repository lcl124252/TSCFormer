...
class pl_model(LightningBaseModel):
    ...
    def forward(self, data_dict):
        def detach_tensors(data):
            if isinstance(data, dict):
                return {k: detach_tensors(v) for k, v in data.items()}
            elif isinstance(data, list):
                return [detach_tensors(item) for item in data]
            elif isinstance(data, torch.Tensor):
                return data.clone().detach()
            else:
                return data
        data_dict_copy = detach_tensors(data_dict)
        return self.model(data_dict_copy)
    ...
    def training_step(self, batch, batch_idx):
        ...
        for key, value in loss_dict.items():
            # dot = make_dot(loss_dict[key], params=dict(self.model.named_parameters()))
            # dot.format = 'pdf'
            # dot.render('computation_graph'+key)
            self.log(
                "train/"+key,
                value.detach(),
                on_epoch=True,
